...
const G1_TEXT = /^[ぁ-んァ-ンヴー0-9 　。、「」？！・＋−＝×÷.cmdLkgt:じふんぷはい]*$/;
const KANJI = /[一-鿿]/;
...
export function validateQuestion(q, stage, label) {
  ...
  if (q.kind === 'keypad') {
    ...
    const cap = stage.numberCap || (stage.bigNumbers ? 10000 : 120);
  ...
  if (q.kind === 'equation-build') {
    ...
    const m = q.math || {};
    if (!['add', 'sub', 'mul', 'div'].includes(m.kind)) err('式づくりなのに math.kind が add/sub/mul/div でない');
    ...
    if (m.kind === 'mul' && !stage.bigNumbers && (m.a > 12 || m.b > 12)) err('かけ算の範囲(12まで)を超える: ' + m.a + '×' + m.b);
    if (m.kind === 'sub' && !(m.a > m.b)) err('引き算なのに a>b でない: ' + m.a + '−' + m.b);
    if (m.kind === 'div') {
      if (m.b > 9) err('わり算のわる数が9を超える: ' + m.b);
      if (m.a % m.b !== 0) err('わり切れないわり算: ' + m.a + '÷' + m.b);
    }
    const expect = m.kind === 'add' ? m.a + m.b : m.kind === 'sub' ? m.a - m.b : m.kind === 'mul' ? m.a * m.b : m.a / m.b;
